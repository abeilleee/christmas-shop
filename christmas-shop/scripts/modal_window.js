...
//Установка баллов (снежинок)

const setScores = (modal, category) => {
    const elem = modal.querySelector(`.${category}`); //Выбираю определенную категорию баллов
...
const openModal = (event) => {
    ...
    if (target.closest('.card')) {
        ...
        setScores(child, 'scores___live');
        setScores(child, 'scores___create');
        setScores(child, 'scores___love');
        setScores(child, 'scores___dream');
